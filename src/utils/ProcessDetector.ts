    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        ...
        if (!isNaN(pid)) {
          processes.push({
            pid,
            command: parts.slice(10).join(" ") || "firefox",
            details: line.trim(),
          });
        }
      }
...
export async function isChromeRunning(): Promise<ProcessInfo[]> {
  try {
    ...
    const command = "ps aux | grep -i 'google chrome\\|chromium' | grep -v grep";
    const { stdout } = await execSimple(command);
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        ...
        if (!isNaN(pid)) {
          processes.push({
            pid,
            command: parts.slice(10).join(" ") || "chrome",
            details: line.trim(),
          });
        }
      }
...
export function getBrowserConflictAdvice(
  ...
  const browserDisplayName = browserName.charAt(0).toUpperCase() + browserName.slice(1);
  
  return `${browserDisplayName} is currently running (${processCount} process${processCount > 1 ? "es" : ""} detected). ` +
