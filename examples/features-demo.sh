...
active_count="$(count_json --url "$TARGET_URL")"
including_expired_count="$(count_json --url "$TARGET_URL" --include-expired)"
...
for browser in chrome firefox safari; do
  browser_count="$(count_json --url "$TARGET_URL" --browser "$browser")"
...
printf '%s\n' "   get-cookie --url $TARGET_URL --detect-jwt --output json"
printf '%s\n' "   get-cookie --url $TARGET_URL --jwt-only --output json"
