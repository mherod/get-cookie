...
describe('FirefoxCookieQueryStrategy - File Discovery', () => {
  ...
  it('should find cookie files in macOS paths', async () => {
    ...
    sync.mockImplementation((pattern: string) => {
      if (pattern.includes('Library/Application Support/Firefox')) {
        return ['/mock/home/Library/Application Support/Firefox/Profiles/abc123/cookies.sqlite'];
      }
      return [];
    });

    await strategy.queryCookies('test-cookie', 'example.com');
    expect(sync).toHaveBeenCalledWith(expect.stringContaining('Library/Application Support/Firefox/Profiles/*/cookies.sqlite'));
  });
  ...
  it('should find cookie files in Linux paths', async () => {
    ...
    await strategy.queryCookies('test-cookie', 'example.com');
    expect(sync).toHaveBeenCalledWith(expect.stringContaining('.mozilla/firefox/*/cookies.sqlite'));
  });
