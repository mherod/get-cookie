// Mock better-sqlite3 first (before any imports that use it)
...
jest.mock("../../sql/DatabaseConnectionManager", () => ({
  getGlobalConnectionManager: jest.fn().mockReturnValue({
...
jest.mock("../../sql/QueryMonitor", () => ({
  getGlobalQueryMonitor: jest.fn().mockReturnValue({
    executeQuery: jest.fn().mockResolvedValue([]),
    executeGet: jest.fn().mockResolvedValue(null),
  }),
}));
